...
def ensure_report_jobs(template: dict) -> dict:
    ...
    for job in template['jobs']:
        if job.get('type') == 'report':
            ...
            if (params.get('template') == 'traditional-json' and 
                params.get('reportFile') == 'seqra_zap_scan_results'):
    ...
    if not has_json_report:
        ...
        logger.info("Added traditional-json report job with reportFile='seqra_zap_scan_results'")
...
def check_template_warnings(template: dict) -> None:
    ...
    if len(contexts) > 1:
        logger.warning(f"Template contains {len(contexts)} contexts. Will use first context by default. "
    ...
    if non_cwe_policies:
        logger.warning(f"Template contains {len(non_cwe_policies)} non-CWE policies that will be kept in output: "
    ...
    if existing_scans:
        logger.warning(f"Template contains {len(existing_scans)} existing activeScan jobs that will be removed: "
...
def select_context(template: dict, context_name: Optional[str]) -> Tuple[dict, int]:
...
def should_include_path(path: str, filters: dict, target_url: str) -> bool:
    ...
    if filters['urls']:
        if not any(pattern.match(full_path) for pattern in filters['urls']):
            return False

    if filters['include']:
        if not any(pattern.match(full_path) for pattern in filters['include']):
            return False

    if filters['exclude']:
        if any(pattern.match(full_path) for pattern in filters['exclude']):
            return False
    return True
...
def run_seqra_scan(project_path: Path) -> Path:
    ...
    cmd = ["seqra", "scan", "--output", str(output_sarif), str(project_path.resolve())]
    try:
        result = subprocess.run(cmd, capture_output=True, text=True, check=True)
        logger.debug(f"Seqra output:\n{result.stdout}")
        if not output_sarif.exists():
            raise RuntimeError(f"Seqra scan completed but SARIF file not found at: {output_sarif}")
        logger.info(f"Seqra scan completed: {output_sarif}")
        return output_sarif
    except subprocess.CalledProcessError as e:
        logger.error(f"Seqra scan failed with exit code {e.returncode}")
        if e.stderr:
            logger.error(f"stderr: {e.stderr}")
        raise RuntimeError(f"seqra scan failed with exit code {e.returncode}") from e
    except FileNotFoundError:
        raise RuntimeError("seqra command not found. Make sure seqra is installed and in PATH")
...
def filter_sarif_by_base(base_sarif_path: Path, new_sarif_path: Path) -> Path:
    ...
    with open(base_sarif_path, 'r') as f:
        base_sarif = json.load(f)
    with open(new_sarif_path, 'r') as f:
...
def generate_automation_yaml(
    ...
    jobs_without_activescan = [
        job for job in template['jobs']
        if job.get('type') != 'activeScan'
    ]
    ...
    output = {
        'env': {
            ...
            **{k: v for k, v in template['env'].items() if k != 'contexts'}
        },
        'jobs': output_jobs
    }
    output_path.parent.mkdir(parents=True, exist_ok=True)
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate ZAP Automation Framework YAML from SARIF"
    )

    input_group = parser.add_mutually_exclusive_group(required=True)
    input_group.add_argument('--sarif', type=Path, help='SARIF file path')
    input_group.add_argument('--project-path', type=Path, help='Project path to scan with Seqra')

    parser.add_argument('--template', type=Path, required=True,
                        help='ZAP automation template with policies')
    parser.add_argument('--target', required=True,
                        help='Target base URL')
    parser.add_argument('--output', type=Path, default=Path('zap-automation.yaml'),
                        help='Output YAML file path')
    parser.add_argument('--base-sarif', type=Path,
                        help='Base SARIF for differential scanning (e.g., from main branch)')
    parser.add_argument('--context-name',
                        help='Context name to use from template (default: first context)')
    ...
    if args.project_path:
        sarif_path = run_seqra_scan(args.project_path)
    else:
